...
const AdminEditProductLogic = (id) => {
  // start Get Data From DataBase >>
  const dispatch = useDispatch();
  useEffect(() => {
    // علشان تشغل async  داخل ال useEffect  لازم تحطهم جوا fun
    const run = async () => {
  ...
  //load the data when the page Renduer >>

  useEffect(() => {
    if (productDetails) {
      setProName(productDetails.title);
      setProDesc(productDetails.description);
      setQty(productDetails.quantity);
      setPriceBefore(productDetails.price);
  ...
  useEffect(() => {
    if (catId != 0) {
      const run = async () => {
        await dispatch(GetSubCatagory(catId));
      };
      run();
    }
  }, [catId]);
