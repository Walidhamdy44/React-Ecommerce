...
import { CreateCatagory } from "../../Redux/Actions/catagoryAction";
import {
  ToastAlertError,
  ToastAlertSucses,
} from "../../components/utilty/ToastAlert";

const AddCatagoryLogic = () => {
  ...
  const handelSubmit = async (e) => {
    ...
    if (name === "" || SelectedFile === null) {
      ToastAlertError("Please Fill Out the Filds");
    } else {
      ...
      await dispatch(CreateCatagory(formData));
      // بعد م البياتات تتحمل حاله التحميل هتبقي true
      setLoading(false);
      setIspress(false);
      // Toast style >>
      ToastAlertSucses("Catagory Add Successfully");
    }
  };
...
export default AddCatagoryLogic;
